...
Effect::Effect()
{
	...
	AddParam( ffglqs::Param::Create( "u_weight", FF_TYPE_HUE, 0.0f ) );
	AddParam( ffglqs::Param::Create( "u_weight_saturation", FF_TYPE_SATURATION, 0.0f ) );
	AddParam( ffglqs::Param::Create( "u_weight_brightness", FF_TYPE_BRIGHTNESS, 0.0f ) );
	AddParam( ffglqs::Param::Create( "u_weight_alpha", FF_TYPE_ALPHA, 1.0f ) );

	SetParamDisplayName( 0, "Luma Point", true );
	SetParamDisplayName( 1, "Luma Width", true );
	SetParamDisplayName( 2, "Luma Blend", true );
	SetParamDisplayName( 3, "Lift", true );
	SetParamDisplayName( 4, "Gamma", true );
	SetParamDisplayName( 5, "Gain", true );
	SetParamDisplayName( 6, "Offset", true );
	SetParamDisplayName( 7, "Weight", true );
}

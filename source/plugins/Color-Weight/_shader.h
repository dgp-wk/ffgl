...
void main()
{
	vec3 weight = vec3(u_red, u_green, u_blue);
	...
	// Calculate a baseline reduction factor based on the max uniform value
	float maxWeight = max(weight.r, max(weight.g, weight.b));
	
	// Create a multiplier that lowers the other channels when one is pushed positive
	vec3 effectiveMultiplier = vec3(1.0) + weight - vec3(maxWeight);

	// Apply the color shift
	color.rgb *= effectiveMultiplier;
